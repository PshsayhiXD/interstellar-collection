const { getChatElements, getUsername, getMessageText, markInitialized, injectStyle, onNewMessages, onChatStateChange } = require("../chatCore");

function chatSearchFilter() {
  const els = getChatElements();
  if (!els) return;
  const { chatBox, chatContent } = els;
  if (markInitialized(chatContent, "searchFilter")) return;
  ...
  const parseQuery = raw => {
    ...
    if (tokens.length === 0) {
      ...
      tokens.forEach((token, i) => {
        ...
        rawValues[token.type === "from" ? "user" : "text"] = raw
          .slice(token.valueStart, end)
          .replace(/:$/, "")
          .trim();
      });
    }
  ...
  const applyMessageFilter = messageNode => {
  ...
  const applyFilter = () => {
    ...
    chatContent.querySelectorAll(".chat-message").forEach(messageNode => {
      if (applyMessageFilter(messageNode)) visibleCount++;
    });
    countLabel.textContent = queryError
      ? "invalid regex"
      : active
  ...
  hint.textContent = "from:USERNAME, text:WORD, from:USERNAME text:WORD or /regex/";
  ...
  updateBarVisibility();
  onChatStateChange(chatBox, updateBarVisibility);
  onNewMessages(chatContent, messages => {
    if (!currentQuery.user && !currentQuery.text && !queryError) return;
    messages.forEach(applyMessageFilter);
  });
}
